...
class Window(Frame):
    ...
    def init_window(self, bg= "Black"):
        ...
        # Labels
        # separated for my own clarity
        # these are used to load and update the text
        self.textWeight = Label(self, bg=None, fg='Black',
            text=str(self.CValue), font=('Arial', 80), height=2, relief=RIDGE)
        self.textWeight.pack(side=RIGHT)
        self.tName = Label(self, bg=None, fg='Black', text=self.CName,
            font=('Arial', 80), height=2, relief=RIDGE)
        self.tName.pack(side=TOP)
        self.HX7 = HX711()
        self.HX7.set_reference_unit_A(reference_unit=float(-26384.03544))

        # Buttons
        ForwardButton = Button(text='calibrate', command=self.selfcalibrate)
        # create test button, remove and replace with GPIO button
        ForwardButton.pack(side=BOTTOM)
        BackButton = Button(text='Backward', command= self.backward)
        # create test button, remove and replace with GPIO button
        BackButton.pack(side=BOTTOM)
        test3 = Button(text='Brake', command= self.neutral)
        # create test button, remove and replace with GPIO button
    ...
    # its important to turn off first so that you arent forcing 
    # the motor to burn out by trying to turn left and right.
    def forward(self):
    ...
    def WriteCSV(self):
        with open('BridgeData.csv', 'a') as csvfile:
            # this is to creat/open a file, 
            # the a is to write at the end of the file.
            filewrite = csv.writer(csvfile)
            filewrite.writerow([self.c1, self.c2, self.c3])  
            # these are placeholders for actual variables.
            iteration = iteration + 1
            # This iteration is not used
